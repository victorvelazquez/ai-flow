...
2. Determine type: features/screens → `feature/`, fixes → `fix/`, refactoring/optimization → `refactor/`, config → `chore/`
3. Generate descriptive slug from changed files
4. Create branch: `git checkout -b [type]/[slug]` (e.g., `feature/add-push-notifications`)
